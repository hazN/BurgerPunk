...
namespace RPGCharacterAnims
{
    public class GUIControls : MonoBehaviour
    {
        ...
        private void Blocking()
        {
			...
			if (!rpgCharacterController.isCasting && !rpgCharacterController.isSitting && !rpgCharacterController.IsActive(HandlerTypes.Relax)) {
                ...
                if (blockGui) {
					...
					if (GUI.Button(new Rect(30, 270, 100, 30), "Block Break")) {
						...
						context.number = 6;
						rpgCharacterController.StartAction(HandlerTypes.GetHit, context);
					}
				}
            }
        }
        ...
        private void Casting()
        {
			...
			if (rpgCharacterController.hasNoWeapon) {
                if (GUI.Button(new Rect(25, 425, 120, 30), "Cast AOE Dual"))
                { rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.AOE.TakeRandom(), Side.Dual)); }
                if (GUI.Button(new Rect(25, 400, 120, 30), "Cast Buff Dual"))
                { rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.Buff.TakeRandom(), Side.Dual)); }
                if (GUI.Button(new Rect(25, 450, 120, 30), "Cast Summon Dual"))
                { rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.Summon.TakeRandom(), Side.Dual)); }
            }
			if (rpgCharacterController.leftWeapon.HasNoWeapon()) {
				if (GUI.Button(new Rect(145, 425, 120, 30), "Cast AOE L"))
				{ rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.AOE.TakeRandom(), Side.Left)); }
				if (GUI.Button(new Rect(145, 400, 120, 30), "Cast Buff L"))
				{ rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.Buff.TakeRandom(), Side.Left)); }
				if (GUI.Button(new Rect(145, 450, 120, 30), "Cast Summon L"))
				{ rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.Summon.TakeRandom(), Side.Left)); }
			}
			if (rpgCharacterController.rightWeapon.HasNoWeapon()) {
				if (GUI.Button(new Rect(265, 425, 120, 30), "Cast AOE R"))
				{ rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.AOE.TakeRandom(), Side.Right)); }
				if (GUI.Button(new Rect(265, 400, 120, 30), "Cast Buff R"))
				{ rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.Buff.TakeRandom(), Side.Right)); }
				if (GUI.Button(new Rect(265, 450, 120, 30), "Cast Summon R"))
				{ rpgCharacterController.StartAction(HandlerTypes.Cast, new CastContext(AnimationVariations.Summon.TakeRandom(), Side.Right)); }
			}
		}
